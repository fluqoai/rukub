...
export function AdminSidebar({ admin }: SidebarProps) {
  ...
  return (
    <aside className="sticky top-0 flex h-screen w-16 flex-shrink-0 flex-col border-l border-ink-900/10 bg-ink-900 text-linen-50 md:w-60">
      {/* Logo */}
      <div className="flex h-16 items-center justify-center gap-2 border-b border-linen-50/10 px-2 md:justify-start md:px-5">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                'flex min-h-11 items-center justify-center gap-3 rounded-xl px-2 py-2 text-sm transition-colors md:justify-start md:px-3',
                active
